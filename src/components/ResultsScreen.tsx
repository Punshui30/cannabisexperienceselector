...
export function ResultsScreen({ recommendations, onCalculate, onBack, onShare }: ResultsProps) {
  ...
  return (
    <div className="w-full flex-grow flex flex-col bg-transparent overflow-hidden font-sans relative">
      ...
      <div className="relative z-20 flex-1 flex flex-col pt-12 px-6">
        
        {/* HEADER */}
        <div className="flex justify-between items-start mb-6">
          <button 
            onClick={onBack}
            className="w-10 h-10 rounded-full bg-black/40 border border-white/10 flex items-center justify-center text-white/60 hover:text-white backdrop-blur-md"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="text-right">
            <div className="inline-block px-3 py-1 rounded-full bg-[#00FFD1]/10 border border-[#00FFD1]/20 backdrop-blur-md">
              <span className="text-[10px] font-bold text-[#00FFD1] tracking-widest uppercase">StrainMath<span className="text-[8px] align-top opacity-60">™</span> Output</span>
            </div>
