...
exports.DOMINANCE_HISTORY = void 0;
exports.calculateCultivarEffect = calculateCultivarEffect;
exports.resetDominanceHistory = resetDominanceHistory;
exports.calculateBlends = calculateBlends;
// ============================================================================
...
const UNKNOWN_TERPENE_COEFFICIENTS = {
    energy: 0.0,
    focus: 0.0,
    mood: 0.0,
    body: 0.0,
...
function calculateDiversity(terpenes) {
...
function calculateBiphasicResponse(concentration, curve) {
...
function calculateSynergyStrength(concentration1, concentration2, curve1, curve2) {
...
function calculateAntagonismStrength(concentration1, concentration2, curve1, curve2) {
    const response1 = calculateBiphasicResponse(concentration1, curve1);
    const response2 = calculateBiphasicResponse(concentration2, curve2);
...
function validateIntent(intent) {
    var _a;
    const validated = Object.assign({}, intent);
    ...
    validated.constraints = {
        maxAnxiety: (_a = intent.constraints.maxAnxiety) !== null && _a !== void 0 ? _a : 0.3,
...
// ============================================================================
function calculateCultivarEffect(cultivar) {
    const effect = {
        energy: 0,
        focus: 0,
        mood: 0,
...
// ============================================================================
function buildBlendProfile(cultivars, ratios) {
    const aggregatedTerpenes = {};
    let thc = 0;
    let cbd = 0;
...
function calculateRisks(profile) {
    ...
    const cbdDampening = Math.min(CBD_RISK_DAMPENING.maxDampening, profile.cbd * CBD_RISK_DAMPENING.perPercent);
...
// ============================================================================
function generateExplanationData(profile, risks, breakdown, cultivarMap, cultivarIds) {
    const explanation = {
...
function getPrimaryEffect(terpene) {
...
// ============================================================================
function evaluateBlend(cultivars, ratios, baseMatchScore, intent) {
    ...
    TERPENE_SYNERGIES.forEach(syn => {
        ...
        if (t1 > 0 && t2 > 0) {
            ...
            const relevance = intent.targetEffects[syn.axis] || 0;
            if (relevance > 0) {
    ...
    TERPENE_ANTAGONISMS.forEach(ant => {
        ...
        if (t1 > 0 && t2 > 0) {
            ...
            const relevance = Math.abs(intent.targetEffects[ant.axis] || 0);
            if (relevance > 0) {
    ...
    const explanationData = generateExplanationData(profile, risks, breakdown, new Map(), cultivars.map(c => c.id));
...
// ============================================================================
exports.DOMINANCE_HISTORY = new Map();
function resetDominanceHistory() {
    exports.DOMINANCE_HISTORY.clear();
    console.log("ENGINE: Dominance history cleared.");
}
// ============================================================================
// MAIN GENERATION FUNCTION
// ============================================================================
function calculateBlends(inventory, intent, exclusionIds // ID-based exclusions only
) {
    ...
    const cultivars = inventory.cultivars
        ...
        .filter(c => !(exclusionIds === null || exclusionIds === void 0 ? void 0 : exclusionIds.includes(c.id)));
    ...
    const cultivarScores = cultivars.map(c => {
        ...
        // Constraints
        if (effect.anxiety > validatedIntent.constraints.maxAnxiety)
            score -= 0.5;
        // DOMINANCE SUPPRESSION (Contextual Penalty)
        const frequency = exports.DOMINANCE_HISTORY.get(c.id) || 0;
        let penalty = 0;
        if (frequency > 0) {
    ...
    const debugRanking = cultivarScores.sort((a, b) => b.score - a.score).slice(0, 10);
    console.log("---------------------------------------------------------");
    console.log(`ENGINE RANKING (Intent: Energy=${validatedIntent.targetEffects.energy} Focus=${validatedIntent.targetEffects.focus})`);
    debugRanking.forEach((cS, idx) => {
        const pStr = cS.penalty > 0 ? ` [Penalty: -${cS.penalty.toFixed(3)} (Freq: ${exports.DOMINANCE_HISTORY.get(cS.id)})]` : "";
        console.log(`  #${idx + 1} ${cS.cultivar.name}: ${cS.score.toFixed(3)} (Base: ${cS.baseScore.toFixed(3)})${pStr}`);
    });
    console.log("---------------------------------------------------------");
    ...
    recommendations.forEach(rec => {
        rec.cultivars.forEach(c => {
            const count = exports.DOMINANCE_HISTORY.get(c.id) || 0;
            exports.DOMINANCE_HISTORY.set(c.id, count + 1);
        });
    });
    return {
        recommendations,
