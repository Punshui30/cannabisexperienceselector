...
module.exports = async function handler(request, response) {
    try {
        console.log('[GEMINI_API_V8.5] Request started');
        ...
        if (!apiKey) {
            console.error('[GEMINI_API_V8.5] Missing GEMINI_API_KEY');
...
Facts: ${tier1Narrative.reasoning}`;
        ...
        if (!fetchFn) {
            console.error('[GEMINI_API_V8.5] fetch is not defined in this environment');
        ...
        console.log('[GEMINI_API_V8.5] Success');
        return response.status(200).json({ ok: true, narrative });

    } catch (err) {
        console.error('[GEMINI_API_V8.5] Global Error:', err.message);
