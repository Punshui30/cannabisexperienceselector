...
module.exports = async function handler(request, response) {
    try {
        console.log('[GEMINI_API_V8.7] Request started');
        ...
        if (!apiKey) {
            console.error('[GEMINI_API_V8.7] Missing GEMINI_API_KEY');
...
Facts: ${tier1Narrative.reasoning}`;
        ...
        if (!fetchFn) {
            console.error('[GEMINI_API_V8.7] fetch is not defined in this environment');
        ...
        if (!geminiRes.ok) {
            ...
            console.error(`[GEMINI_API_V8.7] API Error ${status}:`, errorText);
        ...
        console.log('[GEMINI_API_V8.7] Success');
        return response.status(200).json({ ok: true, narrative });

    } catch (err) {
        console.error('[GEMINI_API_V8.7] Global Error:', err.message);
        return response.status(200).json({
            ok: false,
            error: 'server_error',
            details: err.message
